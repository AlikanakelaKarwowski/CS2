...
public:
    ...
    std::vector<int> inorderVect(TreeNode<NODETYPE>* ptr,std::vector<int>Vec)
    {
        /*
        if(ptr!=nullptr)
        {
            inorderVect(ptr->leftPtr,Vec); // traverse left subtree
            Vec.push_back (ptr->data); // process node
            inorderVect(ptr->rightPtr,Vec); // traverse right subtree
          //  ptr3=ptr3->rightPtr;
        }
        */
        TreeNode<NODETYPE>* ptr2=ptr;
        TreeNode<NODETYPE>* ptr3=ptr;

        while (ptr2 != nullptr) {
          //inorderVect(ptr->leftPtr,Vec); // traverse left subtree
          Vec.push_back (ptr2->data ); // process node
          //inorderVect(ptr->rightPtr,Vec); // traverse right subtree
          ptr2=ptr2->leftPtr;
        ...
        while (ptr3 != nullptr) {
          //inorderVect(ptr->leftPtr,Vec); // traverse left subtree
          Vec.push_back (ptr3->data ); // process node
          //inorderVect(ptr->rightPtr,Vec); // traverse right subtree
          ptr3=ptr3->rightPtr;
    ...
    TreeNode<NODETYPE> *Optimize()
    {

        std::vector<int>Vec;
        Vec=inorderVect(rootPtr,Vec);
        clearTree(rootPtr);//calls the function to remove all the nodes from the tree
        rootPtr=nullptr;
        return OptimizeTree(Vec);
        insertNode(Vec.at(0));//calls the optimize tree function
    }
    ...
    void clearTree(TreeNode<NODETYPE>* rootNode)
    {
        if(rootNode!=nullptr)
        {
            clearTree(rootNode->leftPtr);//delete left
            clearTree(rootNode->rightPtr);//delete right
            delete rootNode;
        }
    }
...
//if it is it will call the right side of the tree
    ...
    TreeNode<NODETYPE>* OptimizeTree(std::vector<int>Vec)
    {
