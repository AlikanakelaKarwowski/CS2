...
public:
   ...
   TreeNode<NODETYPE> *Optimize(std::vector<int> Vec)
   {
        clearTree(rootPtr);//calls the function to remove all the nodes from the tree
        rootPtr=nullptr;
       return OptimizeTree(Vec,0,Vec.size()-1);//calls the optimize tree function
   }
...
TreeNode<NODETYPE>* OptimizeTree(std::vector<int> Vec,int start,int end)
{
if(start>end)
{}
//If there is nothing/nothing left in the tree return
else// find the middle ant insert in the tree. Then go left of the node and right of the node
{
    int mid =(start+end)/2;
    insertNode(mid+1);
   OptimizeTree(Vec,start,mid-1);
   OptimizeTree(Vec,mid+1,end);
